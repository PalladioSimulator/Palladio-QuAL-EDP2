...
public class DoubleBackgroundMemoryListTest {
    ...
    public void basicDeserialization() throws IOException {
        ...
        list = new BackgroundMemoryListImpl<Double, Duration>(tempFile.getAbsolutePath(), doubleSerializer,
        ...
        assertEquals("Value before and after serialization must be the same.", 1.0, list.get(0).doubleValue(unit),
                MAX_DELTA);
    }
    ...
    public void basicDeserializationChunkSize(final long numberElements, final int valueOffset) throws IOException {
        ...
        for (double i = 0; i < numberElements; i++) {
            assertEquals("Value(s) before and after serialization must be the same.", i + 1 + valueOffset,
                    list.get((int) (oldListSize + i)).doubleValue(unit), MAX_DELTA);
        }
    }
    ...
    public void differentChunkSizes() throws IOException {
        ...
        list.close();
        list = new BackgroundMemoryListImpl<Double, Duration>(tempFile.getAbsolutePath(), new DoubleSerializer(),
                BackgroundMemoryListImpl.DEFAULT_CHUNK_SIZE / 4, BinaryRepresentation.DOUBLE, unit);
        assertEquals("Deserialized test list must contain " + numberElements + " element(s).", numberElements,
                list.size(), MAX_DELTA);
        for (double i = 0; i < numberElements; i++) {
            assertEquals("Value(s) before and after serialization must be the same.", i + 1, list.get((int) i)
                    .doubleValue(unit), MAX_DELTA);
        }
        list.close();
    }
    ...
    public void settingElementsInList() throws IOException {
        ...
        list.get(0);
        assertEquals("Changed value not stored correctly.", newValue, list.get(positionOfChange).doubleValue(unit),
                MAX_DELTA);
        ...
        list = new BackgroundMemoryListImpl<Double, Duration>(tempFile.getAbsolutePath(), new DoubleSerializer(),
                BinaryRepresentation.DOUBLE, unit);
        for (int i = 0; i < numberElements; i++) {
            assertEquals("Value(s) before and after serialization must be the same.", values[i], list.get(i)
                    .doubleValue(unit), MAX_DELTA);
        }
    }
}
